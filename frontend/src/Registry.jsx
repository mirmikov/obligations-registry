...
export default function Registry({ user, notify, maintenance, onToggleMaintenance, initialAIScanBatch, onInitialAIScanApplied }) {
  ...
  return <div className={`page registry-page ${tableFullscreen ? 'is-table-fullscreen' : ''}`}>
    ...
    <section className="table-card">
      ...
      {selected.length > 0 && <div className="selection-bar"><span><Check size={16}/>{selected.length} выбрано</span>{selectedItem && <button onClick={() => setHistoryItem(selectedItem)}><Info size={15}/>Информация</button>}{can(user, 'registry.split') && selectedItem && canSplitPayment(selectedItem) && <button onClick={() => setSplitItem(selectedItem)}><Scissors size={15}/>Разбить платёж</button>}{can(user, 'registry.edit') && <button onClick={() => setBulkOpen(true)}>Изменить статус и даты</button>}<button onClick={() => setSelected([])}>Снять выбор</button></div>}
