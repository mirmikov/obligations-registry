...
export default function Registry({ user, notify, maintenance, onToggleMaintenance, announcement, onAnnouncementChanged, initialAIScanBatch, onInitialAIScanApplied }) {
  ...
  return <div className={`page registry-page ${tableFullscreen ? 'is-table-fullscreen' : ''}`}>
    ...
    <section className="table-card">
      ...
      {selected.length > 0 && <div className="selection-bar"><span><Check size={16}/>{selected.length} выбрано</span>{selectedItem && <button onClick={() => setHistoryItem(selectedItem)}><Info size={15}/>Информация</button>}{can(user, 'registry.split') && selectedItem && canSplitPayment(selectedItem) && <button onClick={() => setSplitItem(selectedItem)}><Scissors size={15}/>Разбить платёж</button>}{can(user, 'registry.edit') && <button onClick={() => setBulkOpen(true)}>{selectedApprovalEditable ? 'Изменить статус и даты' : 'Изменить статус / факт оплаты'}</button>}<button onClick={() => setSelected([])}>Снять выбор</button></div>}
      ...
      <tbody>{newRow && <RegistryRow item={newRow} refs={refs} editable approvalEditable={canApproveObligations(user, newRow.legal_entity)} isNew savingCells={savingCells} onCommit={commitNewCell} onStartEdit={startCellEdit} onFinishEdit={finishCellEdit} onCreateCounterparty={entered => openCounterpartyModal(newRow, entered)} onDelete={() => setNewRow(null)}/>} {loading ? <SkeletonRows/> : data.items.length === 0 && !newRow ? <tr><td colSpan="20"><div className="empty-state"><Search size={27}/><strong>Ничего не найдено</strong><span>Измените или сбросьте фильтры</span></div></td></tr> : data.items.map(item => <RegistryRow key={item.id} item={item} refs={refs} editable={can(user, 'registry.edit')} approvalEditable={canApproveObligations(user, item.legal_entity)} selected={selected.includes(item.id)} savingCells={savingCells} onToggle={() => setSelected(s => s.includes(item.id) ? s.filter(id => id !== item.id) : [...s, item.id])} onCommit={commitCell} onStartEdit={startCellEdit} onFinishEdit={finishCellEdit} onCreateCounterparty={entered => openCounterpartyModal(item, entered)} onScanChanged={meta => updateScanMeta(item.id, meta)} notify={notify} onInfo={() => setHistoryItem(item)} onSplit={can(user, 'registry.split') && canSplitPayment(item) ? () => setSplitItem(item) : null} onDelete={can(user, 'registry.delete') ? () => remove(item.id) : null}/>)}</tbody></table></div>
    ...
    {bulkOpen && (
      <BulkModal count={selected.length} refs={refs} approvalEditable={selectedApprovalEditable} onClose={() => setBulkOpen(false)} onSave={async values => { try { await request('/api/obligations/bulk', { method: 'POST', body: JSON.stringify({ ids: selected, ...values }) }); notify('Выбранные строки обновлены'); setBulkOpen(false); setSelected([]); load() } catch (e) { notify(e.message, 'error') } }}/>
    )}
    ...
    {aiScan && <AIScanModal state={aiScan} references={refs} approvalEditable={legalEntity => canApproveObligations(user, legalEntity)} onChange={setAIScan} onRetry={() => aiScanRef.current.click()} onClose={() => !aiScan.loading && !aiScan.saving && setAIScan(null)} onSave={saveAIScan}/>}
